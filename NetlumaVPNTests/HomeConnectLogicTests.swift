...
struct HomeConnectLogicTests {
    ...
    @Test func onlySupportedTabsAreDefined() {
        #expect(NetlumaVPNTab.allCases == [.home, .settings])
        #expect(NetlumaVPNTab.visibleTabs == [.home, .settings])
    }
    ...
    @Test func netlumaVPNManagedProfilesAreRecognized() {
        ...
        let globalProfile = VPNProfile(
            ...
            networkType: VPNNetworkType.tcp,
            origin: VPNProfileOrigin.netlumaVPNGlobal,
            managedServerID: "netlumavpn-mvp-eu-1",
            remarks: "NetlumaVPN Global"
        )

        #expect(localProfile.isNetlumaVPNManaged == false)
        #expect(globalProfile.isNetlumaVPNManaged)
        #expect(globalProfile.managedServerID == "netlumavpn-mvp-eu-1")
    }
}
