...
enum VPNConfigurationParserError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedScheme:
            "Only vless://, vmess://, trojan://, and WireGuard configurations are supported."
        case .unsupportedSecurity(let value):
            "Security '\(value)' is not supported by this app. Use none, tls, or reality."
        case .invalidURL:
            "The configuration link is not valid."
        case .invalidVMessPayload:
            "The VMess configuration payload could not be decoded."
        case .invalidWireGuardValue(let field):
            "The WireGuard value for \(field) is invalid."
        case .missingRequiredField(let field):
            "The configuration link is missing \(field)."
        }
    }
}
