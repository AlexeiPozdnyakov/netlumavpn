...
struct QuickVPNWidgetEntry: TimelineEntry {
    let date: Date
    let state: QuickVPNWidgetState
}

struct QuickVPNWidgetState: Equatable {
    var status: VPNConnectionStatus
    var selectedProfile: VPNProfile?
    var sessionInfo: QuickVPNWidgetSessionInfo?
    var latencyMS: Int?
    ...
    var title: String {
        ...
        case .failed:
            "Connection failed"
        case .disconnected:
            selectedProfile == nil ? "No profile selected" : "Ready to connect"
        }
    }
    ...
    var actionTitle: String {
        ...
        case .disconnecting:
            "Turning off"
        case .failed:
            selectedProfile == nil ? "Add profile" : "Try again"
        case .disconnected:
            selectedProfile == nil ? "Add profile" : "Tap to turn on"
        }
    }
    ...
    var locationSubtitle: String {
        ...
        return "Open QuickVPN to add one"
    }
    ...
    var ipText: String {
        ...
        return sessionInfo?.ipAddress?.nilIfBlank ?? "No IP"
    }
    ...
    static let placeholder = QuickVPNWidgetState(
        status: .disconnected,
        selectedProfile: nil,
        sessionInfo: nil,
        latencyMS: nil
    ...
    static let previewConnected = QuickVPNWidgetState(
        status: .connected,
        ...
        sessionInfo: QuickVPNWidgetSessionInfo(
            ipAddress: "10.8.0.42",
            countryCode: "JP",
            countryName: "Japan",
            city: "Tokyo",
...
struct QuickVPNWidgetSessionInfo: Equatable {
...
private struct QuickVPNWidgetSessionResponse: Decodable {
    var ip: String?
    var countryCode: String?
    var countryName: String?
    var city: String?
...
private struct QuickVPNWidgetSessionInfoService {
    func fetch(timeout: TimeInterval = 1) async throws -> QuickVPNWidgetSessionInfo {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            return QuickVPNWidgetSessionInfo()
        }

        let decoded = try JSONDecoder().decode(QuickVPNWidgetSessionResponse.self, from: data)
        return QuickVPNWidgetSessionInfo(
...
private struct QuickVPNWidgetStateLoader {
    private let profileStorage = ProfileStorage()
    private let vpnController = WidgetVPNController()
    private let diagnostics = ConnectionDiagnostics()
    private let sessionInfoService = QuickVPNWidgetSessionInfoService()
    private let networkDetailTimeout: TimeInterval = 1

    func load(fetchNetworkDetails: Bool) async -> QuickVPNWidgetState {
        ...
        return await QuickVPNWidgetState(
            status: status,
    ...
    private func loadSessionInfo(enabled: Bool) async -> QuickVPNWidgetSessionInfo? {
        guard enabled else {
            return nil
        }
...
struct QuickVPNWidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> QuickVPNWidgetEntry {
        QuickVPNWidgetEntry(date: Date(), state: .placeholder)
    }

    func getSnapshot(
        in context: Context,
        completion: @escaping (QuickVPNWidgetEntry) -> Void
    ) {
        if context.isPreview {
            completion(QuickVPNWidgetEntry(date: Date(), state: .previewConnected))
            return
        }

        Task {
            completion(
                await QuickVPNWidgetEntry(
                    date: Date(),
                    state: QuickVPNWidgetStateLoader().load(fetchNetworkDetails: false)
                )
            )
        }
    }
    ...
    func getTimeline(
        in context: Context,
        completion: @escaping (Timeline<QuickVPNWidgetEntry>) -> Void
    ) {
        Task {
            let entry = await QuickVPNWidgetEntry(
                date: Date(),
                state: QuickVPNWidgetStateLoader().load(fetchNetworkDetails: true)
            )
...
struct QuickVPNWidgetView: View {
    @Environment(\.widgetFamily) private var family

    let entry: QuickVPNWidgetEntry

    var body: some View {
        Group {
            switch family {
            case .systemMedium:
                QuickVPNMediumWidgetView(state: entry.state)
            default:
                QuickVPNSmallWidgetView(state: entry.state)
            }
        }
        .containerBackground(for: .widget) {
            QuickVPNWidgetBackground(status: entry.state.status)
        }
    }
}

private struct QuickVPNSmallWidgetView: View {
    let state: QuickVPNWidgetState
...
private struct QuickVPNMediumWidgetView: View {
    let state: QuickVPNWidgetState
...
private struct PowerIntentButton: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(state.status.isWidgetSessionActive ? "Disconnect VPN" : "Connect VPN")
    }
...
private struct QuickVPNWidgetBackground: View {
    let status: VPNConnectionStatus
...
struct QuickVPNWidget: Widget {
    private let kind = "QuickVPNWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: QuickVPNWidgetProvider()) { entry in
            QuickVPNWidgetView(entry: entry)
        }
        .configurationDisplayName("QuickVPN")
...
@main
struct QuickVPNWidgetBundle: WidgetBundle {
...
#Preview(as: .systemSmall) {
    ...
    QuickVPNWidgetEntry(date: Date(), state: .previewConnected)
}
