...
enum WidgetVPNControllerError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .profileNotFound:
            "Add or select a VPN profile first."
        }
    }
}
...
struct WidgetVPNController {
    ...
    func currentConnectionState(
        ...
        do {
            ...
            let connectionState = manager?.connection.quickVPNWidgetConnectionState
    ...
    private func connect(profile: VPNProfile) async throws -> VPNConnectionStatus {
        ...
        do {
            ...
            let currentStatus = manager.connection.status.quickVPNWidgetStatus
            ...
            let state = manager.connection.quickVPNWidgetConnectionState
    ...
    private func disconnect() async throws -> VPNConnectionStatus {
        ...
        sessionStateStorage.clearConnectionStartedAt()
        let status = manager.connection.status.quickVPNWidgetStatus
...
private extension NEVPNConnection {
    var quickVPNWidgetConnectionState: VPNConnectionState {
        VPNConnectionState(status: status.quickVPNWidgetStatus, connectedDate: connectedDate)
    }
}

private extension NEVPNStatus {
    var quickVPNWidgetStatus: VPNConnectionStatus {
        switch self {
